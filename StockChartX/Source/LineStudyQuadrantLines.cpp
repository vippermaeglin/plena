// LineStudyQuadrantLines.cpp: implementation of the CLineStudyQuadrantLines class.
//
//////////////////////////////////////////////////////////////////////
...
CLineStudyQuadrantLines::CLineStudyQuadrantLines(OLE_COLOR color, LPCTSTR Key, CChartPanel* owner)
{
	objectType = "QuadrantLines";
	nType = lsQuadrantLines;
	objectDescription = "Quadrant Lines";
...
CLineStudyQuadrantLines::~CLineStudyQuadrantLines()
{

}

void CLineStudyQuadrantLines::OnPaint(CDC *pDC)
{
...
void CLineStudyQuadrantLines::OnLButtonUp(CPoint point)
{
...
void CLineStudyQuadrantLines::OnLButtonDown(CPoint point)
{
...
void CLineStudyQuadrantLines::XORDraw(UINT nFlags, CPoint point)
{	
	CPointF pointF = CPointF((double)point.x,(double)point.y);
	...
	else if(nFlags == 3)	//	Finished drawing with the mouse
	{
		...
		pCtrl->OnUserDrawingComplete(lsQuadrantLines, key);
		pCtrl->SaveUserStudies();
...
void CLineStudyQuadrantLines::OnMouseMove(CPoint point)
{
	CPointF pointF = CPointF((double)point.x,(double)point.y);
...
void CLineStudyQuadrantLines::OnRButtonUp(CPoint point)
{	
	if(selected) pCtrl->FireOnItemRightClick(OBJECT_LINE_STUDY, key);	
}
/////////////////////////////////////////////////////////////////////////////

void CLineStudyQuadrantLines::OnDoubleClick(CPoint point)
{	
...
// Workaround
// Simply create a CLineStudyQuadrantLines, 
// set it's x1,y1,x2,y2 and call this.
void CLineStudyQuadrantLines::SnapLine()
{
	startX = 0;
	startY = 0;
	pCtrl->movingObject = false;
...
void CLineStudyQuadrantLines::OnMessage(LPCTSTR MsgGuid, int MsgID)
{
...
void CLineStudyQuadrantLines::DisplayInfo()
{
	if(drawing) return; // The object is being resized
...
void CLineStudyQuadrantLines::DrawLineStudy(CDC* pDC, CRect rect)
{
  ...
  int n;
	...
	for(n = 0; n != 5; ++n){
		...
		if(n == 2) pDC->SelectObject(pOldPen);
		pDC->MoveTo(rect.left, rect.top);
		pDC->LineTo(rect.right, rect.top);
		if(n == 2) pOldPen = pDC->SelectObject(pen);		
		quadLines[n].x1 = rect.left;
		quadLines[n].y1 = rect.top;
		quadLines[n].x2 = rect.right;
		quadLines[n].y2 = rect.top;
	}
...
bool CLineStudyQuadrantLines::IsObjectClicked()
{
	bool clicked = IsRegionClicked();
	if(!clicked){
		for(int n = 0; n != 5; ++n){
			clicked = IsClicked(quadLines[n].x1,quadLines[n].y1,
				quadLines[n].x2,quadLines[n].y2);
			if(clicked) break;
		}
	}
	return clicked;
...
CSeries* CLineStudyQuadrantLines::GetOHLCSeries(LPCSTR ohlc)
{
...
void CLineStudyQuadrantLines::Reset()
{
