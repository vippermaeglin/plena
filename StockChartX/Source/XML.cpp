...
void CXML::operator=( const CXML& markup )
{
...
bool CXML::SetDoc( LPCTSTR szDoc )
{
	// Reset indexes
	m_iPosFree = 1;
	ResetPos();
...
bool CXML::FindElem( LPCTSTR szName )
{
...
bool CXML::FindChildElem( LPCTSTR szName )
{
...
bool CXML::GetOffsets( int& nStart, int& nEnd ) const
{
...
bool CXML::AddElem( LPCTSTR szName, LPCTSTR szValue )
{
...
bool CXML::AddChildElem( LPCTSTR szName, LPCTSTR szValue )
{
...
bool CXML::AddAttrib( LPCTSTR szAttrib, LPCTSTR szValue )
{
...
bool CXML::AddChildAttrib( LPCTSTR szAttrib, LPCTSTR szValue )
{
...
int CXML::x_ParseError( LPCTSTR szError, LPCTSTR szTag )
{
	if ( szTag )
...
bool CXML::x_FindChar( int&n, _TCHAR c ) const
{
...
bool CXML::x_FindToken( CXML::TokenPos& token ) const
{
...
CString CXML::x_GetToken( const CXML::TokenPos& token ) const
{
...
CString CXML::x_GetTagName( int iPos ) const
{
...
bool CXML::x_FindAttrib( CXML::TokenPos& token, LPCTSTR szAttrib ) const
{
...
CString CXML::x_GetAttrib( int iPos, LPCTSTR szAttrib ) const
{
...
CString CXML::x_GetData( int iPos ) const
{
...
CString CXML::x_TextToDoc( LPCTSTR szText, bool bAttrib ) const
{
	...
	while ( cSource )
	{
		...
		if ( (pFound=(_TCHAR*)_tcschr(pFind,cSource)) != NULL )
		{
			...
			_tcscpy(&pDest[nLen],pFound); // For .NET 2003, 6.0 and below
			//_tcscpy_s(&pDest[nLen],nDestSize,pFound); // For .NET 2005 and above
			nLen += _tcslen(pFound);
		}
		else
		{
			_tccpy( &pDest[nLen], pSource ); // For .NET 2003, 6.0 and below
			 //_tccpy_s( &pDest[nLen], nDestSize, NULL, pSource ); // For .NET 2005 and above
			++nLen;
		}
...
CString CXML::x_TextFromDoc( int nLeft, int nRight ) const
{
  ...
  int nMatch;
	...
	while ( nChar <= nRight )
	{
		...
		{			
			...
			_tccpy( &pDest[nLen], &pSource[nChar] ); // For .NET 2003, 6.0 and below
			//_tccpy_s( &pDest[nLen], nDestSize, NULL, &pSource[nChar] ); // For .NET 2005 and above
			nLen += nCharLen;
			nChar += nCharLen;
		}
	}
...
void CXML::x_DocChange( int nLeft, int nReplace, const CString& csInsert )
{
...
void CXML::x_Adjust( int iPos, int nShift )
{
...
int CXML::x_Add( int iPosParent, int iPosBefore, LPCTSTR szName, LPCTSTR szValue )
{
...
int CXML::x_AddAttrib( int iPos, LPCTSTR szAttrib, LPCTSTR szValue )
{
...
HRESULT CXML::SaveFile(LPCTSTR FileName)
{
	...
	text = _T("<?xml version=\"1.0\"?>" + char(13) + char(10));


	text = text + _T("<!DOCTYPE chart PUBLIC \"-//ModulusFE//DTD StockChartX 4.0//EN\"" + char(13) + char(10));
	text = text + _T("\"http://www.modulusfe.com/dtds/stockchartx-4_0.dtd\">" + char(13) + char(10));
	text = text + m_csDoc;

	
#if defined( _UNICODE )	
	int nUTF8Len = WideCharToMultiByte(CP_UTF8,0,text,text.GetLength(),NULL,0,NULL,NULL);
	char* pBuffer = new char[nUTF8Len+1];
	WideCharToMultiByte(CP_UTF8,0,text,text.GetLength(),pBuffer,nUTF8Len+1,NULL,NULL);
	try
	{
...
HRESULT CXML::LoadFile(LPCTSTR FileName)
{
...
//	End of the revision
	...
	int nWideSize = MultiByteToWideChar(CP_UTF8,0,(const char*)pBuffer,nFileLen,m_csDoc.GetBuffer(nFileLen),nFileLen);
	m_csDoc.ReleaseBuffer(nWideSize);
#else
	m_csDoc = (char*)pBuffer;
#endif

	// Convert newlines to CRLFs for CEdit
	CString csCRLFText;
	const _TCHAR* pSource = (LPCTSTR)m_csDoc;
	_TCHAR* pDest = csCRLFText.GetBuffer(m_csDoc.GetLength() * 2);
