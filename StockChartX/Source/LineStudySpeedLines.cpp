// LineStudySpeedLines.cpp: implementation of the CLineStudySpeedLines class.
//
//////////////////////////////////////////////////////////////////////
...
CLineStudySpeedLines::CLineStudySpeedLines(OLE_COLOR color, LPCTSTR Key, CChartPanel* owner)
{

	// Object type and description
	objectType = "SpeedLines";
	nType = lsSpeedLines;
	objectDescription = "Speed Resistance Lines";
...
CLineStudySpeedLines::~CLineStudySpeedLines()
{

}

void CLineStudySpeedLines::OnPaint(CDC *pDC)
{
...
void CLineStudySpeedLines::OnLButtonUp(CPoint point)
{
...
void CLineStudySpeedLines::OnLButtonDown(CPoint point)
{
	CPointF pointF = CPointF((double)point.x,(double)point.y);
	if(drawing) return; // The object is being resized
...
void CLineStudySpeedLines::XORDraw(UINT nFlags, CPoint point)
{	
	CPointF pointF = CPointF((double)point.x,(double)point.y);
	pointF.x = RealPointX(pointF.x);
	pointF.y = RealPointY(pointF.y);
	...
	if(nFlags == 1){ // First point
		...
		if (pointF.x < startX+15) pointF.x = RealPointX(startX+15);				
...
void CLineStudySpeedLines::OnMouseMove(CPoint point)
{
	CPointF pointF = CPointF((double)point.x,(double)point.y);
...
void CLineStudySpeedLines::OnRButtonUp(CPoint point)
{	
	if(selected) pCtrl->FireOnItemRightClick(OBJECT_LINE_STUDY, key);	
}
/////////////////////////////////////////////////////////////////////////////

void CLineStudySpeedLines::OnDoubleClick(CPoint point)
{	
	if (selected) {
...
// Simply create a CLineStudySpeedLines, 
// set it's x1,y1,x2,y2 and call this.
void CLineStudySpeedLines::SnapLine()
{
	startX = 0;
	startY = 0;
	pCtrl->movingObject = false;
...
void CLineStudySpeedLines::OnMessage(LPCTSTR MsgGuid, int MsgID)
{
	if(MsgGuid != guid) {
		return;
	}
...
void CLineStudySpeedLines::DisplayInfo()
{
	if(drawing) return; // The object is being resized
...
void CLineStudySpeedLines::DrawLineStudy(CDC *pDC, CRectF rect)
{
	...
	//new calculus for the speedLines
	double dy = abs(yt2 - yt1);
	if(yt1 > yt2){
		yt3 = yt1 - dy/3;
		yt4 = yt1 - (2*dy)/3;
	}else {
...
bool CLineStudySpeedLines::IsObjectClicked()
{
