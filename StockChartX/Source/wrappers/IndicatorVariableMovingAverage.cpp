// IndicatorVariableMovingAverage.cpp: implementation of the CIndicatorVariableMovingAverage class.
//
//////////////////////////////////////////////////////////////////////
...
CIndicatorVariableMovingAverage::CIndicatorVariableMovingAverage(LPCTSTR name, int type, int members, CChartPanel* owner)
{
	szName = name;
	ownerPanel = owner;
	pCtrl = owner->pCtrl;
	...
	indicatorType = indVariableMovingAverage;

}

CIndicatorVariableMovingAverage::~CIndicatorVariableMovingAverage()
{
	CIndicator::OnDestroy();
}


void CIndicatorVariableMovingAverage::SetParamInfo(){
...
BOOL CIndicatorVariableMovingAverage::Calculate()
{
...
//	End Of Revision
	...
	CMovingAverage ta;
	pInd = ta.VariableMovingAverage(pNav, pSource, paramInt[1], szName);
	...
	for(int n = 0; n < size; ++n){
		...
		if(n < paramInt[1] * 3 || value > pSource->getValue(n) * 2 ||  value < pSource->getValue(n) / 2){ // 9/5/08
			value = NULL_VALUE;
		}
