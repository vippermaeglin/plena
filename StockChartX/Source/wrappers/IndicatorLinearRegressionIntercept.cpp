// IndicatorLinearRegressionIntercept.cpp: implementation of the CIndicatorLinearRegressionIntercept class.
//
//////////////////////////////////////////////////////////////////////
...
CIndicatorLinearRegressionIntercept::CIndicatorLinearRegressionIntercept(LPCTSTR name, int type, int members, CChartPanel* owner)
{
	szName = name;
	ownerPanel = owner;
	pCtrl = owner->pCtrl;
	...
	indicatorType = indLinearRegressionIntercept;

}

CIndicatorLinearRegressionIntercept::~CIndicatorLinearRegressionIntercept()
{
	CIndicator::OnDestroy();
}


void CIndicatorLinearRegressionIntercept::SetParamInfo(){
...
BOOL CIndicatorLinearRegressionIntercept::Calculate()
{
...
//	End Of Revision
	...
	if(pCtrl->m_Language==1){
		szTitle = "RLI" + CString("("+paramInt[1]+")");
	}
	else{
		szTitle = "LRI" + CString("("+paramInt[1]+")");
	}	
	...
	for(int n = 0; n < size; ++n){
		if(n < paramInt[1]){
			...
			value = pInd->getValue("Intercept", n + 1);
		}
