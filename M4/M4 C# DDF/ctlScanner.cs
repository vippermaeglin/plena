  ...
  public partial class ctlScanner : UserControl, IDataSubscriber
  {
    ...
    private void EnableControls(bool Enable, bool UpdateUI)
    {
      ...
      if ((Enable))
      {
        ...
        cmdScanner.Border.BaseColor = Color.Lime;
      }
      else
      {
        cmdScanner.Text = "&Stop Scanner";
        cmdScanner.Border.BaseColor = Color.Red;
      }

      cmdScanner.Border.Update();
    }
    ...
    private void LoadScanner()
    {
      ...
      try
      {
        data = svc.GetUserData(frmMain.ClientId, frmMain.ClientPassword, frmMain.LicenseKey, "Scanner Settings: " + scanName);
      }
      catch (Exception)
      {
    ...
    //Bar udpates
    public void BarUpdate(string Symbol, Periodicity BarType, 
    ...
    private void grdResults_CellClick(object sender, DataGridViewCellEventArgs e)
    {
      ...
      try
      {
        ...
        else if (e.ColumnIndex == 6) //Chart
        {
          ...
          m_frmMain.CreateNewChart(selection);
