...
namespace M4
{
  public partial class ctlAlert : UserControl, IDataSubscriber
  {
    ...
    public ctlAlert(frmMain oMain, ctlData oData)
    {
      InitializeComponent();
    ...
    private void LoadAlert()
    {
      ...
      {
        if (cboPeriodicity.Items[n].Text == text[1])
        {
      ...
      {
        if (cmbPortfolio.Items[n].Text == text[9])
        {
      ...
      {
        if (cboExchanges.Items[n].Text == text[11])
        {
    ...
    private void BroadcastAlert(string Description, string Side)
    {
      ...
      try
      {
        ds = svc.GetAlerts(frmMain.ClientId, frmMain.ClientPassword, frmMain.LicenseKey);
        if (ds == null)
        {
          return;
        }
      ...
      if (count > 99)
      {
        ...
        svc.SetUserData(frmMain.ClientId, frmMain.ClientPassword, frmMain.LicenseKey, firstKey, "");
      }
      ...
      try
      {
        svc.SetUserData(frmMain.ClientId, frmMain.ClientPassword, frmMain.LicenseKey, key, data);
      }
      catch (Exception)
      {
        return;
